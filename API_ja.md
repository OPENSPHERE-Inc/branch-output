...
- 録画中（ファイル分割有効時）: フォーマットが変化した時点でファイルスプリットが即座に実行されます。
  録画がまだデータを書き込んでいない場合（録画開始直後など）は、ファイルスプリットの代わりに、
  データの書き込み後に新しいフォーマットで録画がリスタートされます。
...
- **遅延反映**: 録画が遷移中（開始直後、pending、分割、リスタート処理中）の場合、オーバーライドは
  同期的には反映されず、後から適用されます。通常は 1 秒程度で適用され、録画開始直後は最初のデータが
  書き込まれた後に適用されます。プロシージャ呼び出しは即座に返却され、反映タイミングの通知はありません。

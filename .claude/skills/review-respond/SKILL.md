...
When enabled, in Step 3, once each finding fix is complete, the change is committed to git.
...
- Relationship to build verification: commits are made after Step 4 (Build Verification). If a build error occurs, include that fix in the commit as well.
...
When enabled, wait for user confirmation immediately after the estimate result table is printed to the console in Step 2. Do not confirm immediately after triage.
...
- `triage` (set by: review-respond Step 1) — Value format: `🔧 Will Fix (assignee: {specialist}) — {triage rationale}` / `🚫 Won't Fix — {reason no action is needed}`.
- `estimate` (set by: review-respond Step 2) — Value format: `▶️ Maintain — Cost: {S/M/L}, Future: {S/M/L}, Signals: {none\|a,b,c,d,e,f}` / `🔻 Downgrade — Cost: ..., Future: ..., Signals: ... — {downgrade rationale}` / `🚧 Alternative — Cost: ..., Future: ..., Signals: ... — FIXME insertion: {direction}`.
- `status` (set by: review-respond Step 3) — Value format: `🟢 Fixed — {concise description of what was fixed}`.
...
- `triage` / Step 1 / `Will Fix` → 🔧 (confirmed as a fix target)
- `triage` / Step 1 / `Won't Fix` → 🚫 (confirmed as no action needed)
- `estimate` / Step 2 / `Maintain` → ▶️ (maintain triage verdict and proceed with the fix)
- `estimate` / Step 2 / `Downgrade` → 🔻 (overturn triage verdict; do not fix; no alternative)
- `estimate` / Step 2 / `Alternative` → 🚧 (triage verdict overturned, but addressed via FIXME comment or similar alternative)
...
### Sub-Agent Prompt Completeness

Do not omit, shorten, or summarize the sub-agent prompts (procedure / input paths / output schema / return-value spec) defined in this SKILL. Pass the full template verbatim to the Agent tool. The only embeddable parts are variable parameters (`{document_path}` / `{tmp_dir}` / assigned ids, etc.) and round-specific override appendices supplied by the caller (review-rounds, etc.).
...
Each sub-agent looks up finding-body information (description / location / existing metadata, etc.) by `id` directly from around the METADATA markers in the review document.
...
## Step 1 — Triage (Delegated to the Triage Sub-Agent)

Triage is delegated to a **single sub-agent that is separate from the specialist agents performing fixes** (to avoid bias). The triage Sub Reads the review document directly and performs finding extraction, stage classification, and triage verdict in one pass. Have the verdicts **Written to a file** so verdict bodies are not loaded into the leader's context.
...
```
You are responsible for primary triage of the review document. Read {document_path}, render stage classification and triage verdict for each finding, and Write the result to {tmp_dir}/triage.json.
...
Render verdicts only for findings whose stage is pending_triage or feedback. For other stages, only count them; do not render a triage verdict.
...
High-severity exception: even with a Won't Fix verdict, when severity is Critical / Major, explicitly note "recommend separate PR" in the rationale (e.g., "Won't Fix — existing-code bug. Recommend addressing in a separate PR").
...
{tmp_dir}/triage.json: {items: [{id, verdict, assignee (null for Won't Fix), reason, memo_value}], will_fix_count, wontfix_count, by_stage: {<stage>: <int>}}

memo_value format:
- Will Fix: "🔧 Will Fix (assignee: {assignee}) — {reason}"
- Won't Fix: "🚫 Won't Fix — {reason}"

Return value: {path, will_fix_count, wontfix_count, by_stage, by_assignee: [{assignee, ids: [id, ...]}]} (by_assignee groups Will Fix items by assignee; do not include reason / memo_value bodies in the return value)
```

2. Receive the return value (`{path, will_fix_count, wontfix_count, by_stage, by_assignee}`) from the sub-agent. Do not read the triage body.
...
If `will_fix_count == 0`, skip Steps 2, 3, and 4 and proceed to Step 5 (aggregation).

## Step 2 — Estimate (Delegated in Parallel to Specialist Sub-Agents)

Loop over the `by_assignee` array received in Step 1. For each `{assignee, ids}` pair, launch a specialist sub-agent in parallel via `Agent(subagent_type=assignee, prompt=...)` (the agent definition's persona and specialty perspective load automatically). Each sub-agent batch-estimates its assigned ids and Writes `{tmp_dir}/estimates/{id}.json` for each id.

Launch procedure:

1. Example prompt (do not include persona):

```
Batch-estimate the fix cost for assigned findings {ids} (investigation and verdict only; do not perform the fix).

Input (look up by id == "{finding-id}"):
- Review document {document_path} — read description / location / severity from around the METADATA markers
- {tmp_dir}/triage.json — look up verdict / reason from the items array by id
...
Return value: [{id, verdict}, ...] (one entry for every assigned id)
```
...
When both Maintain and Alternative are 0 (all Downgrade): skip Steps 3 and 4 and proceed to Step 5 (aggregation). Record them as Downgrade in the summary.

## Step 3 — Fix (Delegated in Parallel to Specialist Sub-Agents)

Cross-reference the `by_assignee` from Step 1 with the `{id → verdict}` map from Step 2. Extract only the ids whose `verdict` is `Maintain` or `Alternative`, then re-group them by assignee. For each `{assignee, ids}` pair, launch a fix sub-agent via `Agent(subagent_type=assignee, prompt=...)` (the agent definition's persona and specialty perspective load automatically).
...
```
Sequentially fix assigned findings {ids}.

Input (look up by id == "{finding-id}"):
- Review document {document_path} — read description / location from around the METADATA markers
- {tmp_dir}/triage.json — look up verdict / reason from the items array by id
...
Parallelism constraints (when handling multiple ids):
- Multiple ids affecting the same file must be processed sequentially (to avoid write conflicts).
- Ids affecting different files may be processed in parallel.
...
Return value: [{id, path}, ...] (one entry for every assigned id)
```

### Parallelism (Per Assignee)

- Different assignees are launched concurrently in parallel.
- Handling of ids within the same assignee follows the parallelism constraints in the prompt above (the specialist Sub itself decides).

### Status Saved to Intermediate Files

The leader only collects the return values (a list of `[{id, path}, ...]`); status bodies are not loaded into context.
...
5. If the maximum attempts are reached without success, present `error_summary` to the user, exit the loop, and proceed to Step 5 (aggregator sub-agent).
...
## Step 5 — Reflect into the Review Document (Delegated to Aggregator Sub-Agent)

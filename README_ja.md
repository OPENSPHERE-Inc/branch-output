...
- OBS Studio の配信・録画・仮想カメラ・リプレイバッファの状態と連動可能
- 録画ファイルの手動分割 (1.0.5より)
- 録画のポーズ・アンポーズ (1.0.5より)
- 録画へのチャプチャーマーカー追加（Hybrid MP4のみ対応） (1.0.5より)
- フィルター単位の独立したリプレイバッファ (1.0.8より)
- フィルタ入力モード：フィルタチェーン上の映像をキャプチャして出力可能（実験的機能、1.0.8より）
- ソースがプログラム出力に表示されていない場合のブランキング・ミュート (1.0.8より)
- ステータスドック設定のプロファイル別保存 (1.0.8より)
- 映像クロッピング：相対（マージン）・絶対（リージョン）モード対応、ライブプレビュー矩形表示 (1.0.9より)
- フレームレートディバイダー：出力フレームレートを分周して低減（1/2, 1/3, 1/4, 1/5, 1/6） (1.0.9より)
- アウトプット種別ごとの個別開始・停止制御：「Individual」インターロックモードおよびステータスドックのアウトプット別有効化・無効化チェックボックス (1.0.9より)
- スクリプトから録画保存ファイル名フォーマットをオーバーライドするプロシージャハンドラ（[APIリファレンス](./API_ja.md)）(1.0.9より)
- スクリプトからリプレイバッファ保存ファイル名フォーマットをオーバーライドするプロシージャハンドラ（[APIリファレンス](./API_ja.md)）(1.0.9より)
- 各種ホットキー
  - フィルターEnable/Disable
  - 手動録画分割、ポーズ・アンポーズ、チャプターの追加 (1.0.5より)
  - 全体Enable/Disable
  - 全体録画分割、全体ポーズ・アンポーズ、全体チャプター追加 (1.0.5より)
  - リプレイバッファ保存、全体リプレイバッファ保存 (1.0.8より)
  - アウトプット別の有効化・無効化：配信（全スロット一括／スロット別）、録画、リプレイバッファ (1.0.9より)
- スタジオモード対応
  - **Branch Output はスタジオモードのプログラム出力を無視し、常にプレビューを出力に使用します**
...
## 3. 録画・リプレイバッファのファイル名を動的にオーバーライドしたい
